...
from .models import Event, Category, Participant, Review
...
def event_update(request, pk):
    event = get_object_or_404(Event, pk=pk)
    if event.author != request.user and not request.user.is_staff:
...
def event_delete(request, pk):
    ...
    if not event.author or event.author != request.user:
